...
struct Body {
    ...
    vector<int> list_node1, list_node2;
};

//Data for each cell
struct Cell {
    int ID2, parent2,NumNodes=0,level,neighbors[6]={-1,-1,-1,-1,-1,-1},Ncell;
    vector<int> members, child,scell;
...
public:
    ...
    //void traverse_tree(Parameters params,vector<Cell> &cell, Body body[], int root,int node);
...
void traverse_tree(Parameters params,vector<Cell> &cell,Body body[],int root,int node){
    int i,theta=params.theta; ///////Get from input params
    float D=sqrt(pow((body[node].r1[0]-cell[root].r2[0]),2.0)+pow((body[node].r1[1]-cell[root].r2[1]),2.0)+pow((body[node].r1[2]-cell[root].r2[2]),2.0));
    float r=sqrt(pow(cell[root].rd[0],2.0)+pow(cell[root].rd[1],2.0)+pow(cell[root].rd[2],2.0));
    if (D<(r/theta)){
        if (cell[root].child.size()!=0)
            for(i=0; i<cell[root].child.size(); ++i)
                body[node].list_node1.push_back(cell[root].child[i]);
        if(cell[root].scell.size()!=0)
            for(i=0; i<cell[root].scell.size(); ++i)
                traverse_tree(params,cell,body,cell[root].scell[i],node);}
    else
        body[node].list_node2.push_back(root);
}
...
template <class type> void octree<type>::compute_force(Parameters params,vector<Cell> &cell,Body body[],int node){
    ...
    if(body[node].list_node1.size()>=1)
        for(int j=0; j<3; ++j)
            for (int i=0; i<body[node].list_node1.size(); ++i)
                body[node].force[j]=body[node].force[j]+(G*body[node].m1* body[body[node].list_node1[i]].m1) * (body[node].r1[j]-body[body[node].list_node1[i]].r1[j])/pow((1+pow((body[node].r1[0]-body[body[node].list_node1[i]].r1[0]),2.0)+pow((body[node].r1[1]-body[body[node].list_node1[i]].r1[1]),2.0)+pow((body[node].r1[2]-body[body[node].list_node1[i]].r1[2]),2.0)),1.5);
    
    
    if(body[node].list_node2.size()>=1)
        for(int j=0; j<3; ++j)
            for (int i=0; i<body[node].list_node2.size(); ++i)
                body[node].force[j]=body[node].force[j]+(G*body[node].m1* cell[body[node].list_node2[i]].m2) * (body[node].r1[j]-cell[body[node].list_node2[i]].r2[j])/pow((1+pow((body[node].r1[0]-cell[body[node].list_node2[i]].r2[0]),2.0)+pow((body[node].r1[1]-cell[body[node].list_node2[i]].r2[1]),2.0)+pow((body[node].r1[2]-cell[body[node].list_node2[i]].r2[2]),2.0)),1.5);
...
int main(int argc, const char * argv[]){
    ...
    boost::timer timer;
    for(i=0;i<N; ++i)
        traverse_tree(params,cell,body,0,i);
