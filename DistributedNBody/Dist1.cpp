              ...
              min_index_(0)
    {
      ...
      if(flag==1){
            for(std::size_t j=0; j<size_mem; ++j)
                data_[j]=(*body)[cell[cell_IDs].members[j]];
...
//    void do_work(std::size_t np, std::size_t nl);
...
struct stepper : hpx::components::client_base<stepper, stepper_server>
{
   ...
   // hpx::future<void> do_work(std::size_t np, std::size_t nl){
        return hpx::async(do_work_action(), get_gid(), np, nl);
    }
};
//////////////////////////////////////////////////////////////////////////////

void stepper_server::initialize_body(int N, Body body1)
{
...
{
    cell[n].child.erase(cell[n].child.begin(),cell[n].child.begin()+cell[n].child.size());
    cell[n].scell.erase(cell[n].scell.begin(),cell[n].scell.begin()+cell[n].scell.size());
    int p = n * 8;
    for (int i = 1; i < 9; ++i)
    {
        cell[p + i].ID2 = p + i;
        ...
        cell[p + i].parent2 = n;
        cell[p+i].members.erase(cell[p+i].members.begin(),cell[p+i].members.begin()+cell[p+i].members.size());
            ...
            unwrapped(
                    [ce0, cell_ID, dim, dir](partition_data const& m) -> partition_data
                    {
                        ...
                        for (std::size_t i = 0; i < size; ++i){
                            ...
                            if(m[i].ID1>=0){
                                if (dir == -1) {
                                    if ((*body)[m[i].ID1].r1[dim] < cell[cell_ID].boundary[2 * dim])
                                        next[i].ID1 = m[i].ID1; }

                                if(dir==1){
                                    if ((*body)[m[i].ID1].r1[dim] > cell[cell_ID].boundary[2 * dim + 1])
                                        next[i].ID1 = m[i].ID1; }
                            }
                       }
                        return next;
...
//void stepper_server::do_work(std::size_t np, std::size_t nl)
{
    using hpx::lcos::local::dataflow;
    using hpx::util::unwrapped;
...
*/
        ...
        From_B[0] = dataflow(hpx::launch::async, &stepper_server::New_Members, B[0], B[0], ID_lc);
        From_F[0] = dataflow(hpx::launch::async, &stepper_server::New_Members, F[0], F[0], ID_lc);
        From_L[0] = dataflow(hpx::launch::async, &stepper_server::New_Members, L[0], L[0], ID_lc);
        From_R[0] = dataflow(hpx::launch::async, &stepper_server::New_Members, R[0], R[0], ID_lc);
        From_U[0] = dataflow(hpx::launch::async, &stepper_server::New_Members, U[0], U[0], ID_lc);
        From_D[0] = dataflow(hpx::launch::async, &stepper_server::New_Members, D[0], D[0], ID_lc);
...
int hpx_main(boost::program_options::variables_map& vm)
{
    ...
    //hpx::future<void> result = step.do_work(np, nl);
   ...
   if (0 == hpx::get_locality_id())
    ...
    //        s[0].get_data(partition_server::cell0).wait();       
