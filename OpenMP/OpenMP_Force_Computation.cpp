...
struct Parameters{
    ...
    double theta=0.5, G1=6.673*pow(10.0,-11.0);
};
...
void apply_changes(vector<Cell_Data> &cell,int n, Body body[]){
...
bool InCube(vector<Cell_Data> &cell,Body body[],int node, int p){
    for (int i=0; i<3; ++i)
...
void det_boundary_subcube(vector<Cell_Data> &cell,int n){
double a1,a2,a3,b1,b2,b3,c1,c2,c3; Cell_Data cell1[8];
...
public:
    octree() {};
    void root(vector<Cell_Data> &cell, Body body[], int n);
    void strc(vector<Cell_Data> &cell,Parameters params,Body body[], vector<int> &cell_arrange, int n);
    void neighbor(vector<Cell_Data> &cell,Body body[], vector<int> &cell_arrange);
    void cell_list(vector<Cell_Data> &cell,Parameters params,Body body[], vector<int> &cell_arrange, int N);
    //void traverse_tree(vector<Cell_Data> &cell,Parameters params, Body body[], int root,int node, int root2);
    void compute_force(vector<Cell_Data> &cell,Parameters params,Body body[],int node);
    void new_tree(vector<Cell_Data> &cell,Parameters params,Body body[],vector<int> &cell_arrange,int parent,int node);
    void insert_node(vector<Cell_Data> &cell,Parameters params,Body body[],vector<int> &cell_arrange,int parent, int node);
};

//Computing Center of mass and position of cell
template <class type> void octree<type>::root(vector<Cell_Data> &cell,Body body[], int n){
    vector<float> CM(3,0);
    for (int j=0; j<cell[n].NumNodes; ++j){
...
template <class type> void octree<type>::strc(vector<Cell_Data> &cell,Parameters params,Body body[], vector<int> &cell_arrange, int n){
    ...
    for(i=1; i<9; ++i){
        ...
        for(j=0; j<cell[n].members.size(); ++j){
            if(InCube(cell,body,cell[n].members[j],i+p)==true){
    ...
    for(i=1; i<9; ++i)
        if(cell[i+p].members.size()>params.th)
            tree.strc(cell,params,body,cell_arrange,i+p);}

//Modifying octree when position of nodes are changed (1)
template <class type> void octree<type>::insert_node(vector<Cell_Data> &cell,Parameters params,Body body[],vector<int> &cell_arrange,int parent, int node){
    octree<float> tree;
    ...
    if(cell[parent].NumNodes==params.th+1){
        ...
        //tree.strc(cell,params,body,cell_arrange,parent);
        }

    if(cell[parent].NumNodes>params.th){
        int p=cell[parent].Ncell;
        for(int i=1; i<9; ++i){
            if(InCube(cell,body,node,i+p)==true)
                tree.insert_node(cell,params,body, cell_arrange, i+p, node);}}}

//Modifying octree when position of nodes are changed (2)
template <class type> void octree<type>::new_tree(vector<Cell_Data> &cell,Parameters params,Body body[],vector<int> &cell_arrange,int parent,int node){
    int k=-1; octree<float> tree;
    if (parent!=0){
        for(int j=0; j<6; ++j)
            if(cell[parent].neighbors[j]!=-1 && k==-1)
            if(InCube(cell,body,node,cell[parent].neighbors[j])==true){
                k=1; int temp=cell[parent].neighbors[j];
                tree.insert_node(cell,params,body,cell_arrange,temp,node);}

        if(k==-1 && cell[parent].parent2!=0)
            tree.new_tree(cell,params,body,cell_arrange,cell[parent].parent2,node);}}

//Determining neighbors of each cell (left,right,up,down,back,front)
template <class type> void octree<type>::neighbor(vector<Cell_Data> &cell,Body body[], vector<int> &cell_arrange){
    ...
    if (ratio<params.theta){
        ...
        if(cell[root2].scell.size()>=1)
            for(i=0; i<cell[root2].scell.size(); ++i)
                traverse_tree(cell,params,body,root,node,cell[root2].scell[i]);}
    else
        cell[root].list_cell2.push_back(root2);}

//creating interaction list for each cell (1)
template <class type> void octree<type>::cell_list(vector<Cell_Data> &cell,Parameters params,Body body[], vector<int> &cell_arrange, int N) {

    octree<float> tree;
    for (int i = 0; i < N; ++i)
        if (cell[body[i].parent].list_cell1.size() == 0 && cell[body[i].parent].list_cell2.size() == 0)
            traverse_tree(cell,params, body, body[i].parent, i, 0);
}


//computing force, new position and new velocity
template <class type> void octree<type>::compute_force(vector<Cell_Data> &cell,Parameters params,Body body[],int node){
    vector<float> a(3,0); double t=params.time_step; 
    double G=params.G1;
    int parent=body[node].parent;
    vector<double> solution2(3,0), solution1(3,0);


    if(cell[parent].list_cell1.size()>=1){
        for(int j=0; j<3; ++j)
            for (int i=0; i<cell[parent].list_cell1.size(); ++i)
                solution1[j]=body[node].force[j]+(G*body[node].m1* body[cell[parent].list_cell1[i]].m1) * (body[node].r1[j]-body[cell[parent].list_cell1[i]].r1[j])/pow((1+pow((body[node].r1[0]-body[cell[parent].list_cell1[i]].r1[0]),2.0)+pow((body[node].r1[1]-body[cell[parent].list_cell1[i]].r1[1]),2.0)+pow((body[node].r1[2]-body[cell[parent].list_cell1[i]].r1[2]),2.0)),1.5);}

    if(cell[body[node].parent].list_cell2.size()>=1){
        for(int j=0; j<3; ++j)
            for (int i=0; i<cell[body[node].parent].list_cell2.size(); ++i)
                solution2[j]=body[node].force[j]+(G*body[node].m1* cell[cell[parent].list_cell2[i]].m2) * (body[node].r1[j]-cell[cell[parent].list_cell2[i]].r2[j])/pow((1+pow((body[node].r1[0]-cell[cell[parent].list_cell2[i]].r2[0]),2.0)+pow((body[node].r1[1]-cell[cell[parent].list_cell2[i]].r2[1]),2.0)+pow((body[node].r1[2]-cell[cell[parent].list_cell2[i]].r2[2]),2.0)),1.5);}

	for (int j=0; j<3;++j)
		body[node].force[j]=solution1[j]+solution2[j];
...
int main(int argc, const char * argv[]){
    ...
    tree.strc(cell,params,body,cell_arrange,0);
    tree.neighbor(cell,body,cell_arrange);
    tree.cell_list(cell,params,body,cell_arrange,N);

    boost::timer timer;

    //In terminal I have added these options too: OMP_NUM_THREADS=16, OMP_PLACES=0,3,1,2, OMP_PROC_BIND=spread or close in order to
    ...
    #pragma omp parallel for schedule (dynamic,10) num_threads(4) 
        for (int i=0; i<N; ++i){
            tree.compute_force(cell,params,body,i);
            if((InCube(cell,body,i,body[i].parent)==false)){
                cell[body[i].parent].NumNodes=cell[body[i].parent].NumNodes-1;

                if(InCube(cell,body,i,0)==true)
                    tree.new_tree(cell,params,body,cell_arrange,body[i].parent,i);}
	    }}
