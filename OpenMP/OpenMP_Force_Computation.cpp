...
void compute2(int node){
    ...
    if (N1 >= 1) {
        //#pragma omp parallel for schedule (dynamic,3) num_threads(4) //sometime it increases time!
        for(int i=0; i<N1; ++i)
...
int main(int argc, char** argv){
    ...
    //In terminal I have added these options too: OMP_NUM_THREADS=4, OMP_PLACES=0,3,1,2, OMP_PROC_BIND=spread or close in order to
